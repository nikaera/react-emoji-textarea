...
const EmojiTextarea = React.forwardRef<HTMLTextAreaElement, EmojiTextAreaProps>(({
    ...
    if (match && match[1]) {
      ...
      if (/^:[a-zA-Z0-9_+-]+:$/.test(match[0])) {
        try {
          ...
          const exact = results.find((emoji: any) => emoji.id === match[1]);
          if (exact) {
  ...
  const handlePickerSelect = (emoji: any) => {
    ...
    setTimeout(() => {
      if (textareaRef.current) {
        textareaRef.current.focus();
        textareaRef.current.selectionStart = textareaRef.current.selectionEnd = before.length + emoji.native.length;
      }
    }, 0);
  };
          ...
          {suggestResults.map((emoji, idx) => {
            ...
            return (
              <div
                ...
                onMouseDown={e => { e.preventDefault(); handleSuggestSelect(emoji); }}
                onMouseEnter={() => setSuggestIndex(idx)}
              >
                <span style={{ fontSize: 22, marginRight: 12, flexShrink: 0 }}>{emoji.skins[0].native}</span>
                <span style={{ fontSize: 14, color: idx === suggestIndex ? '#1264A3' : '#616061', opacity: 0.85 }}>
                  :{startIdx !== -1 ? (
                    <>
                      {before}
                      <span style={{ background: '#ffe066', borderRadius: 3 }} data-testid="emoji-suggest-id">{match}</span>
                      {after}
                    </>
                  ) : (
                    id
